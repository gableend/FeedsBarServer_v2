...
export function itemCanonicalHash(feedId: string, urlNormalized: string, guid?: string | null) {
  const key = guid ? `${feedId}|guid|${guid}` : `${feedId}|url|${urlNormalized}`;
  return sha256Hex(key);
}

export function normalizeTitle(t: string | null | undefined): string | null {
  if (!t) return null;
